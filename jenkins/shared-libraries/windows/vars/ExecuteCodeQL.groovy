...
def call(Org, Repo, Branch, Language, BuildCommand, Token, InstallCodeQL) {
    ...
    powershell """
        ...
        Write-Output "The SARIF category has been configured to swa-\$Env:LANGUAGE\$Env:SEP\$Env:CWD"

        Write-Output "Analyzing database"
        if("\$Env:INSTALL_CODEQL" -eq "true") {
            Invoke-Expression ".\\codeql\\codeql database analyze --no-download '\$Env:DATABASE_PATH' \$Env:CODEQL_THREADS_FLAG \$Env:CODEQL_RAM_FLAG --sarif-category 'swa-\$Env:LANGUAGE\$Env:SEP\$Env:CWD' --format sarif-latest --output '\$Env:SARIF_FILE' '\$Env:QL_PACKS'"
        } else {
            Invoke-Expression "codeql database analyze --no-download '\$Env:DATABASE_PATH' \$Env:CODEQL_THREADS_FLAG \$Env:CODEQL_RAM_FLAG --sarif-category 'swa-\$Env:LANGUAGE' --format sarif-latest --output '\$Env:SARIF_FILE' '\$Env:QL_PACKS'"
        }
