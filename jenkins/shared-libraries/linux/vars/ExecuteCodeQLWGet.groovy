def call(org, repo, branch, language, buildCommand, token, installCodeQL) {
    ...
    sh '''
        ...
        if [ ! -x "\$(command -v \$command)" ]; then
            echo "jq not found on PATH, installing jq"
            wget -q --header "${AUTHORIZATION_HEADER}" -O jq https://github.com/jqlang/jq/releases/download/jq-1.7/jq-linux-amd64
            chmod +x jq
            jq_command="${WORKSPACE}/jq"
        fi
        ...
        if [ "${INSTALL_CODEQL}" = false ]; then
            ...
            if [ "${ENABLE_TLS_NO_VERIFY}" = true ]; then
                id=\$(wget -no-check-certificate --quiet --tries=3 \
                --header "${AUTHORIZATION_HEADER}" \
                --header "Accept: application/vnd.github.raw" \
                -O- \
                "https://api.github.com/repos/github/codeql-action/contents/src/defaults.json" | "$jq_command" -r .bundleVersion)

                echo "Downloading CodeQL version '\$id'"
                wget -no-check-certificate --quiet --tries=3 -O "${WORKSPACE}/codeql.tgz" \
                --header "${AUTHORIZATION_HEADER}" \
                ...
                echo "Downloading CodeQL version '\$id'"
                wget --quiet --tries=3 -O "${WORKSPACE}/codeql.tgz" \
                --header "${AUTHORIZATION_HEADER}" \
                "https://github.com/github/codeql-action/releases/download/\$id/codeql-bundle-linux64.tar.gz"
                tar -xf "${WORKSPACE}/codeql.tgz" --directory "${WORKSPACE}"
                rm "${WORKSPACE}/codeql.tgz"
