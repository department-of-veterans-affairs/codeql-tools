def call(org, repo, branch, language, buildCommand, token, installCodeQL) {
    ...
    sh """
        ...
        if [ -z "$BUILD_COMMAND" ]; then
            ...
            /var/lib/jenkins/workspace/mhv-sm-clinician-messagemgmt-portlet/development/codeql/codeql database create "$DATABASE_PATH" --language="$LANGUAGE" --source-root .
        else
            echo "Build command specified, using '$BUILD_COMMAND'"
            /var/lib/jenkins/workspace/mhv-sm-clinician-messagemgmt-portlet/development/codeql/codeql database create "$DATABASE_PATH" --language="$LANGUAGE" --source-root . --command="$BUILD_COMMAND"
        fi
        echo "Database initialized"

        echo "Analyzing database"
        /var/lib/jenkins/workspace/mhv-sm-clinician-messagemgmt-portlet/development/codeql/codeql database analyze "$DATABASE_PATH" --no-download --sarif-category "$LANGUAGE" --format sarif-latest --output "$SARIF_FILE" --search-path="codeql-queries" "codeql/$LANGUAGE-queries:codeql-suites/$LANGUAGE-security-extended.qls"
        echo "Database analyzed"

        echo "Generating CSV of results"
        /var/lib/jenkins/workspace/mhv-sm-clinician-messagemgmt-portlet/development/codeql/codeql database interpret-results "$DATABASE_PATH" --format=csv --output="codeql-scan-results.csv"
        echo "CSV of results generated"

        echo "Uploading SARIF file"
        commit=\$(git rev-parse HEAD)
        /var/lib/jenkins/workspace/mhv-sm-clinician-messagemgmt-portlet/development/codeql/codeql github upload-results \
        --repository="$ORG/$REPO" \
        ...
        /var/lib/jenkins/workspace/mhv-sm-clinician-messagemgmt-portlet/development/codeql/codeql database bundle "$DATABASE_PATH" --output "$DATABASE_BUNDLE"
