def call(repo, language) {
    ...
    sh """
        ...
        "\$command" database analyze "${DATABASE_PATH}" --no-download ${CODEQL_THREADS_FLAG} ${CODEQL_RAM_FLAG} --sarif-category "ois-${LANGUAGE}\${SEP}\${SUBDIR}" --format "sarifv2.1.0" --output "${SARIF_FILE}" "${QL_PACKS}"
